...
// POST /media/upload-file, then attach the /media/<id> URL. Digital => endDate null.
product, _ := z.Organizer.CreateEvent(ctx, zatabox.WithBody(map[string]interface{}{
	"title": "Synth Presets Vol. 1", "category": "downloadable-files",
	"startDate": "2026-08-01T00:00:00Z", "endDate": nil,
	"timezone": "Africa/Lagos", "venueType": "online",
	"onlineLink": "https://api.zatabox.com/media/501", // default delivery, buyer-only
}))
var p struct{ ID string `json:"id"` }
_ = json.Unmarshal(product, &p)
_, _ = z.Organizer.CreateTicket(ctx, p.ID, zatabox.WithBody(map[string]interface{}{
	...
	"accessUrl": "https://api.zatabox.com/media/502", "accessNote": "Zip password: serum", // per-tier link
}))
_, _ = z.Organizer.PublishEvent(ctx, p.ID)
...
_, _ = z.Organizer.CreateEvent(ctx, zatabox.WithBody(map[string]interface{}{
	"title": "Intro to Mixing", "category": "online-course",
	"startDate": "2026-08-01T00:00:00Z", "endDate": nil,
	"timezone": "Africa/Lagos", "venueType": "online",
	"courseType": "syllabus", "certificateTemplate": "cert-1",
	"syllabus": []map[string]interface{}{{"title": "Gain staging", "duration": "1-4 hours",
		"skillLevel": "Beginner", "format": "Self-paced",
		"lessonPdf": "/media/601", "lessonVideo": "/media/602",
		"writeup": "Notes for enrolled buyers only."}},
	"exam": map[string]interface{}{"enabled": true, "passMarkPct": 70, "questions": []map[string]interface{}{
		{"prompt": "Ideal average level?", "options": []string{"-18 dBFS", "0 dBFS"}, "correctIndex": 0}}},
}))

// Take a course (learner). Lesson media in the payload are short-lived signed /media URLs.
_, _ = buyer.Courses.Mine(ctx)                        // { courses: [...] }
_, _ = buyer.Courses.Get(ctx, courseEventID)          // meta + progress + lessons
_, _ = buyer.Courses.CompleteLesson(ctx, courseEventID, "0") // zero-based lesson index
_, _ = buyer.Courses.SubmitExam(ctx, courseEventID, zatabox.WithBody(map[string]interface{}{
	"answers": []map[string]interface{}{{"questionId": "q1", "selectedIndex": 0}}}))
pdf, _, _ := buyer.Courses.Certificate(ctx, courseEventID) // PDF bytes + content type
_ = os.WriteFile("certificate.pdf", pdf, 0o644)

// Filter by product type. course=true narrows digital to online courses.
_, _ = z.Events.List(ctx, zatabox.WithQuery(map[string]interface{}{"productType": "digital", "course": true}))
_, _ = z.Events.Search(ctx, zatabox.WithQuery(map[string]interface{}{"q": "mixing", "productType": "digital", "course": true}))
```
...
| Method | Endpoint | Description |
| --- | --- | --- |
| `List(ctx, opts...) (json.RawMessage, error)` | `GET /api/v1/events` | List and search published public events (cursor-paginated). Filters: q, category, city, and the read-only facets productType (event\|reservation\|digital) and course=true (courses within digital). |
| `Search(ctx, opts...) (json.RawMessage, error)` | `GET /api/v1/search` | Full-text + faceted event search. Same q/category/city/productType/course facets as events.list, plus date_from/date_to; each result carries category + read-only productType. |
| `Get(ctx, slug, opts...) (json.RawMessage, error)` | `GET /api/v1/events/:slug` | Event detail by slug (organizer info, schedule, active ticket types; carries category + read-only productType). |
...
| Method | Endpoint | Description |
| --- | --- | --- |
| `Seatmap(ctx, eventId, opts...) (json.RawMessage, error)` | `GET /api/v1/public/events/:eventId/seatmap` | Get the seat map: sections, seats with x/y coordinates, stage/label elements, and the ticket type + price per section. |
| `Seats(ctx, eventId, opts...) (json.RawMessage, error)` | `GET /api/v1/public/events/:eventId/seats` | Live seat availability snapshot { sold, blocked, held } any seat id not listed is available. |
| `LiveURL(eventId, query...) string` | `GET /api/v1/public/events/:eventId/seats/live` | Server-Sent Events stream of live seat availability (a snapshot on connect, then seat_update events). |
| `Hold(ctx, eventId, opts...) (json.RawMessage, error)` | `POST /api/v1/public/events/:eventId/seats/hold` | Hold (replace) the seats a buyer is selecting for an opaque holder token; returns { held, failed }. |
| `ReleaseHold(ctx, eventId, opts...) (json.RawMessage, error)` | `DELETE /api/v1/public/events/:eventId/seats/hold` | Release a holder's seat holds (omit seatIds in the body to release all of them). |
...
| `Venues(ctx, opts...) (json.RawMessage, error)` | `GET /api/v1/organizer/venues` | Browse published venue seat maps you can attach to an event (interactive seating). |
| `GetEventSeating(ctx, id, opts...) (json.RawMessage, error)` | `GET /api/v1/organizer/events/:id/seating` | Get an event's interactive-seating setup: attached map, section→ticket-type pricing, and blocked/sold seats. |
| `AttachSeating(ctx, id, opts...) (json.RawMessage, error)` | `PUT /api/v1/organizer/events/:id/seating` | Attach a published venue map to the event (body: venueMapId, optional mode/holdTtlSeconds). |
| `DetachSeating(ctx, id, opts...) (json.RawMessage, error)` | `DELETE /api/v1/organizer/events/:id/seating` | Remove interactive seating from the event (only before any seat sells). |
| `SetSeatingPricing(ctx, id, opts...) (json.RawMessage, error)` | `PUT /api/v1/organizer/events/:id/seating/pricing` | Map each venue section to one of the event's ticket types (body: mappings:[{venueSectionId, ticketTypeId}]). |
| `SetSeatingBlocks(ctx, id, opts...) (json.RawMessage, error)` | `POST /api/v1/organizer/events/:id/seating/blocks` | Block or unblock seats for the event (body: block:[seatId], unblock:[seatId]). |
...
| Method | Endpoint | Description |
| --- | --- | --- |
| `Mine(ctx, opts...) (json.RawMessage, error)` | `GET /api/v1/courses/mine` | List the buyer's enrolled courses, each with course meta and a progress summary (started courses first). No lesson content. |
| `Get(ctx, eventId, opts...) (json.RawMessage, error)` | `GET /api/v1/courses/:eventId` | Get one course's full learner payload: meta + progress + lessons (syllabus, with completed flags and signed media) or a unified content block. |
| `CompleteLesson(ctx, eventId, index, opts...) (json.RawMessage, error)` | `POST /api/v1/courses/:eventId/lessons/:index/complete` | Mark a syllabus lesson complete (idempotent, zero-based index). Completing the last requirement finishes the course and may issue a certificate. |
| `UncompleteLesson(ctx, eventId, index, opts...) (json.RawMessage, error)` | `DELETE /api/v1/courses/:eventId/lessons/:index/complete` | Reverse a syllabus lesson completion (does not revoke an already-issued certificate). |
| `Complete(ctx, eventId, opts...) (json.RawMessage, error)` | `POST /api/v1/courses/:eventId/complete` | Complete a unified course (unified courses only): marks the single learning unit complete. |
| `Exam(ctx, eventId, opts...) (json.RawMessage, error)` | `GET /api/v1/courses/:eventId/exam` | Get the course exam to take. Correct answers and explanations are stripped server-side. |
| `SubmitExam(ctx, eventId, opts...) (json.RawMessage, error)` | `POST /api/v1/courses/:eventId/exam/submit` | Submit the exam (body: answers:[{questionId, selectedIndex}]) for server-side grading; a pass drives completion + certificate issuance. |
| `Certificate(ctx, eventId, opts...) ([]byte, string, error)` | `GET /api/v1/courses/:eventId/certificate` | Download the issued completion certificate as a PDF (404 CERTIFICATE_NOT_ISSUED until earned). |
