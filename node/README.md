...
- **Complete** every one of the **98 REST endpoints** is a typed method.
...
`auth`, `events`, `seating`, `organizer`, `eventCustomization`, `tickets`, `orders`,
`payments`, `checkin`, `community`, `growth`, `users`, `courses`, `integrations`,
`webhooks`.
...
### Sell a digital product (delivery link)

`productType` is derived from `category`; a digital `category` makes the event a
download. Host the file (multipart `POST /media/upload-file`) and drop the returned
`/media/<id>` URL into the event `onlineLink` and/or a per-tier `accessUrl`:

```js
const product = await zatabox.organizer.createEvent({
  title: 'Synth Presets Vol. 1', category: 'downloadable-files',
  startDate: '2026-08-01T00:00:00Z', endDate: null,   // digital: open-ended
  timezone: 'Africa/Lagos', venueType: 'online',
  onlineLink: 'https://api.zatabox.com/media/501',     // default delivery, buyer-only
  onlineLinkNote: 'Import into Serum.',
});
await zatabox.organizer.createTicket(product.id, {
  ...
  accessUrl: 'https://api.zatabox.com/media/502',       // per-tier link, buyer-only
  accessNote: 'Zip password: serum',
});
await zatabox.organizer.publishEvent(product.id);
```

### Author an online course

Set `category: 'online-course'` and the course fields. A `syllabus` lesson carries
uploaded `/media/<id>` links (upload with `private=1` for paid content) and an
optional final `exam`:

```js
const course = await zatabox.organizer.createEvent({
  ...
  syllabus: [{
    title: 'Gain staging', duration: '1-4 hours', skillLevel: 'Beginner', format: 'Self-paced',
    lessonPdf: '/media/601',            // POST /media/upload-file?field=pdf&private=1
    lessonVideo: '/media/602',          // field=video&private=1
...
await zatabox.organizer.createTicket(course.id, {
  name: 'Enrolment', type: 'general', price: 15000, currency: 'NGN',
  quantityTotal: -1, saleStart: '2026-07-01T00:00:00Z', saleEnd: '2026-12-31T00:00:00Z',
});
await zatabox.organizer.publishEvent(course.id);
```

### Take a course (learner)

Enrolled buyers list, work through lessons, sit the exam and download the
certificate. Lesson media in the payload are short-lived signed `/media` URLs:

```js
const { courses } = await buyer.courses.mine();
const detail = await buyer.courses.get(courses[0].eventId);   // meta + progress + lessons
await buyer.courses.completeLesson(detail.eventId, 0);        // zero-based lesson index

const exam = await buyer.courses.exam(detail.eventId);        // no answer key
await buyer.courses.submitExam(detail.eventId, {
  answers: exam.questions.map((q) => ({ questionId: q.id, selectedIndex: 0 })),
});
const { data, filename } = await buyer.courses.certificate(detail.eventId); // PDF bytes
require('fs').writeFileSync(filename || 'certificate.pdf', data);
```

### Filter by product type

`productType` (`event` | `reservation` | `digital`) is a read-only facet on
`events.list` and `events.search`; `course=true` narrows digital to courses:

```js
await zatabox.events.list({ productType: 'digital', course: true });
await zatabox.events.search({ q: 'mixing', productType: 'digital', course: true });
```
...
| Method | Endpoint | Description |
| --- | --- | --- |
| `list(query?, opts?)` | `GET /api/v1/events` | List and search published public events (cursor-paginated). Filters: q, category, city, and the read-only facets productType (event\|reservation\|digital) and course=true (courses within digital). |
| `search(query?, opts?)` | `GET /api/v1/search` | Full-text + faceted event search. Same q/category/city/productType/course facets as events.list, plus date_from/date_to; each result carries category + read-only productType. |
| `get(slug, query?, opts?)` | `GET /api/v1/events/:slug` | Event detail by slug (organizer info, schedule, active ticket types; carries category + read-only productType). |
...
| Method | Endpoint | Description |
| --- | --- | --- |
| `seatmap(eventId, query?, opts?)` | `GET /api/v1/public/events/:eventId/seatmap` | Get the seat map: sections, seats with x/y coordinates, stage/label elements, and the ticket type + price per section. |
| `seats(eventId, query?, opts?)` | `GET /api/v1/public/events/:eventId/seats` | Live seat availability snapshot { sold, blocked, held } any seat id not listed is available. |
| `liveUrl(eventId, query?)` | `GET /api/v1/public/events/:eventId/seats/live` | Server-Sent Events stream of live seat availability (a snapshot on connect, then seat_update events). |
| `hold(eventId, body?, opts?)` | `POST /api/v1/public/events/:eventId/seats/hold` | Hold (replace) the seats a buyer is selecting for an opaque holder token; returns { held, failed }. |
| `releaseHold(eventId, body?, opts?)` | `DELETE /api/v1/public/events/:eventId/seats/hold` | Release a holder's seat holds (omit seatIds in the body to release all of them). |
...
| `venues(query?, opts?)` | `GET /api/v1/organizer/venues` | Browse published venue seat maps you can attach to an event (interactive seating). |
| `getEventSeating(id, query?, opts?)` | `GET /api/v1/organizer/events/:id/seating` | Get an event's interactive-seating setup: attached map, section→ticket-type pricing, and blocked/sold seats. |
| `attachSeating(id, body?, opts?)` | `PUT /api/v1/organizer/events/:id/seating` | Attach a published venue map to the event (body: venueMapId, optional mode/holdTtlSeconds). |
| `detachSeating(id, body?, opts?)` | `DELETE /api/v1/organizer/events/:id/seating` | Remove interactive seating from the event (only before any seat sells). |
| `setSeatingPricing(id, body?, opts?)` | `PUT /api/v1/organizer/events/:id/seating/pricing` | Map each venue section to one of the event's ticket types (body: mappings:[{venueSectionId, ticketTypeId}]). |
| `setSeatingBlocks(id, body?, opts?)` | `POST /api/v1/organizer/events/:id/seating/blocks` | Block or unblock seats for the event (body: block:[seatId], unblock:[seatId]). |
...
| Method | Endpoint | Description |
| --- | --- | --- |
| `mine(query?, opts?)` | `GET /api/v1/courses/mine` | List the buyer's enrolled courses, each with course meta and a progress summary (started courses first). No lesson content. |
| `get(eventId, query?, opts?)` | `GET /api/v1/courses/:eventId` | Get one course's full learner payload: meta + progress + lessons (syllabus, with completed flags and signed media) or a unified content block. |
| `completeLesson(eventId, index, body?, opts?)` | `POST /api/v1/courses/:eventId/lessons/:index/complete` | Mark a syllabus lesson complete (idempotent, zero-based index). Completing the last requirement finishes the course and may issue a certificate. |
| `uncompleteLesson(eventId, index, body?, opts?)` | `DELETE /api/v1/courses/:eventId/lessons/:index/complete` | Reverse a syllabus lesson completion (does not revoke an already-issued certificate). |
| `complete(eventId, body?, opts?)` | `POST /api/v1/courses/:eventId/complete` | Complete a unified course (unified courses only): marks the single learning unit complete. |
| `exam(eventId, query?, opts?)` | `GET /api/v1/courses/:eventId/exam` | Get the course exam to take. Correct answers and explanations are stripped server-side. |
| `submitExam(eventId, body?, opts?)` | `POST /api/v1/courses/:eventId/exam/submit` | Submit the exam (body: answers:[{questionId, selectedIndex}]) for server-side grading; a pass drives completion + certificate issuance. |
| `certificate(eventId, query?, opts?) → BinaryResponse` | `GET /api/v1/courses/:eventId/certificate` | Download the issued completion certificate as a PDF (404 CERTIFICATE_NOT_ISSUED until earned). |
