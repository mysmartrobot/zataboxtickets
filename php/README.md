...
$product = $z->organizer->createEvent([
...
$z->organizer->createTicket($product['id'], [
...
$z->organizer->publishEvent($product['id']);
...
$course = $z->organizer->createEvent([
...
$z->organizer->publishEvent($course['id']);
...
$buyer->courses->completeLesson($detail['eventId'], 0);     // zero-based lesson index
$exam = $buyer->courses->exam($detail['eventId']);          // no answer key
$buyer->courses->submitExam($detail['eventId'], ['answers' =>
    array_map(fn($q) => ['questionId' => $q['id'], 'selectedIndex' => 0], $exam['questions'])]);
$pdf = $buyer->courses->certificate($detail['eventId']);    // ['data' => bytes, 'filename', 'contentType']
file_put_contents($pdf['filename'] ?? 'certificate.pdf', $pdf['data']);
...
| Method | Endpoint | Description |
| --- | --- | --- |
| `list($query = null, array $opts = [])` | `GET /api/v1/events` | List and search published public events (cursor-paginated). Filters: q, category, city, and the read-only facets productType (event\|reservation\|digital) and course=true (courses within digital). |
| `search($query = null, array $opts = [])` | `GET /api/v1/search` | Full-text + faceted event search. Same q/category/city/productType/course facets as events.list, plus date_from/date_to; each result carries category + read-only productType. |
| `get($slug, $query = null, array $opts = [])` | `GET /api/v1/events/:slug` | Event detail by slug (organizer info, schedule, active ticket types; carries category + read-only productType). |
...
| Method | Endpoint | Description |
| --- | --- | --- |
| `seatmap($eventId, $query = null, array $opts = [])` | `GET /api/v1/public/events/:eventId/seatmap` | Get the seat map: sections, seats with x/y coordinates, stage/label elements, and the ticket type + price per section. |
| `seats($eventId, $query = null, array $opts = [])` | `GET /api/v1/public/events/:eventId/seats` | Live seat availability snapshot { sold, blocked, held } any seat id not listed is available. |
| `liveUrl($eventId, $query = null)` | `GET /api/v1/public/events/:eventId/seats/live` | Server-Sent Events stream of live seat availability (a snapshot on connect, then seat_update events). |
| `hold($eventId, $body = null, array $opts = [])` | `POST /api/v1/public/events/:eventId/seats/hold` | Hold (replace) the seats a buyer is selecting for an opaque holder token; returns { held, failed }. |
| `releaseHold($eventId, $body = null, array $opts = [])` | `DELETE /api/v1/public/events/:eventId/seats/hold` | Release a holder's seat holds (omit seatIds in the body to release all of them). |
...
| `venues($query = null, array $opts = [])` | `GET /api/v1/organizer/venues` | Browse published venue seat maps you can attach to an event (interactive seating). |
| `getEventSeating($id, $query = null, array $opts = [])` | `GET /api/v1/organizer/events/:id/seating` | Get an event's interactive-seating setup: attached map, section→ticket-type pricing, and blocked/sold seats. |
| `attachSeating($id, $body = null, array $opts = [])` | `PUT /api/v1/organizer/events/:id/seating` | Attach a published venue map to the event (body: venueMapId, optional mode/holdTtlSeconds). |
| `detachSeating($id, $body = null, array $opts = [])` | `DELETE /api/v1/organizer/events/:id/seating` | Remove interactive seating from the event (only before any seat sells). |
| `setSeatingPricing($id, $body = null, array $opts = [])` | `PUT /api/v1/organizer/events/:id/seating/pricing` | Map each venue section to one of the event's ticket types (body: mappings:[{venueSectionId, ticketTypeId}]). |
| `setSeatingBlocks($id, $body = null, array $opts = [])` | `POST /api/v1/organizer/events/:id/seating/blocks` | Block or unblock seats for the event (body: block:[seatId], unblock:[seatId]). |
...
| Method | Endpoint | Description |
| --- | --- | --- |
| `mine($query = null, array $opts = [])` | `GET /api/v1/courses/mine` | List the buyer's enrolled courses, each with course meta and a progress summary (started courses first). No lesson content. |
| `get($eventId, $query = null, array $opts = [])` | `GET /api/v1/courses/:eventId` | Get one course's full learner payload: meta + progress + lessons (syllabus, with completed flags and signed media) or a unified content block. |
| `completeLesson($eventId, $index, $body = null, array $opts = [])` | `POST /api/v1/courses/:eventId/lessons/:index/complete` | Mark a syllabus lesson complete (idempotent, zero-based index). Completing the last requirement finishes the course and may issue a certificate. |
| `uncompleteLesson($eventId, $index, $body = null, array $opts = [])` | `DELETE /api/v1/courses/:eventId/lessons/:index/complete` | Reverse a syllabus lesson completion (does not revoke an already-issued certificate). |
| `complete($eventId, $body = null, array $opts = [])` | `POST /api/v1/courses/:eventId/complete` | Complete a unified course (unified courses only): marks the single learning unit complete. |
| `exam($eventId, $query = null, array $opts = [])` | `GET /api/v1/courses/:eventId/exam` | Get the course exam to take. Correct answers and explanations are stripped server-side. |
| `submitExam($eventId, $body = null, array $opts = [])` | `POST /api/v1/courses/:eventId/exam/submit` | Submit the exam (body: answers:[{questionId, selectedIndex}]) for server-side grading; a pass drives completion + certificate issuance. |
| `certificate($eventId, $query = null, array $opts = [])` | `GET /api/v1/courses/:eventId/certificate` | Download the issued completion certificate as a PDF (404 CERTIFICATE_NOT_ISSUED until earned). |
